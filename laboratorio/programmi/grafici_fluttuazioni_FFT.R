...
setwd("/home/ferrero/Desktop/FFT07_rawdata")
# setwd("/home/ferrero/Dropbox/odour")

#################INIZIO#################      7
dati_flut = as.matrix(read.table("campo_flut_z=2.000m_ds=1.240.txt",sep=""))

#leggo il campo di concentrazione media
fconc<-file("concentrazione_z=2.000m_ds=1.240.txt","r")
#
#n° punti grigliato
...
#q=3.31592 kg/10min   Mp=q/Np*  DT          0.0000000000003   Np=10  DT=0.1
 
#massa=3.31592/600/10*0.1   #Trial 7
massa=6.83275/600/10*0.1   #Trial 46
#kg
rinorm=0.0000000000003/massa
...
ymis=y_rel
#ROTAZIONE
# 150.57 Trial 7 ; 107.55 Trial 46
#ang=-(270-150.57)/180*3.14   #T7
ang=-(270-107.55)/180*3.14    #T46
...
#points(XT1P1,YT1P1+250)
loc=array(c(XT1P1,YT1P1),dim = c(100,100))
aa=conc_media1[,,1]
maxaa=max(aa)
obj=list(x=cx,y=cy,z=aa)
...
bb=fluttuazioni1[,,1]
obj=list(x=cx,y=cy,z=bb)
lookfl=interp.surface(obj,loc)
lookint=array(data=NA,dim=100)
for(i in 1:100){

if(lookcm[i]!=0 & !is.na(lookcm[i]) ){
  lookint[i]=lookfl[i]/lookcm[i]
...
stat0=matrix(NA,nrow=100,ncol=4)
stat0[,]=c(sqrt(xmis**2+ymis**2),lookcm,lookfl,lookint)
#stat0[,]=c(sqrt(xmis**2+ymis**2),lookcm)
#stat1b=t(stat0)
stat1b=stat0
...
#rownames(stat1b) = c("DIST ","CMEAN","  CSD","  I  ")
 stat1b = format(stat1b,digits = 2,scientific=T)
...
#mis=read.table("Uttenweiler_experiments_data.csv",sep=";",skip=1)
#cinter=statutto[,2:4]
#cinter=t(stat1b)
cinter=stat0
...
meanDm=mean(mea,na.rm=TRUE)
meanMm=mean(lookcm,na.rm=TRUE)
FBm=(meanDm-meanMm)/(meanDm+meanMm)*2
NMSEm=mean((mea-lookcm)^2,na.rm=TRUE)/meanDm/meanMm
Rm=cor(mea,lookcm, use="pairwise.complete.obs")
maxDm=max(mea,na.rm=TRUE)
maxMm=max(lookcm,na.rm=TRUE)

conto=0
Sf2=0
for (i in seq(length(mea))){
         if(mea[i]!=0 & !is.na(mea[i]) &!is.na(lookcm[i])) {
        conto=conto+1
	if (lookcm[i]/mea[i] < 2 & lookcm[i]/mea[i] > 0.5){
		Sf2=Sf2+1
	 }
	}
        }
...
for (i in seq(length(mea))){
  if(mea[i]!=0 & !is.na(mea[i]) &!is.na(lookcm[i])) {
    conto=conto+1
    if (lookcm[i]/mea[i] < 5 & lookcm[i]/mea[i] > 0.2){
      Sf5=Sf5+1
    }
  }
}
...
meanDs=mean(sig,na.rm=TRUE)
meanMs=mean(lookfl,na.rm=TRUE)
FBs=(meanDs-meanMs)/(meanDs+meanMs)*2
NMSEs=mean((sig-lookfl)^2,na.rm=TRUE)/meanDs/meanMs
Rs=cor(sig,lookfl, use="pairwise.complete.obs")
maxDs=max(sig,na.rm=TRUE)
maxMs=max(lookfl,na.rm=TRUE)

conto=0
Sf2=0
for (i in seq(length(sig))){
         if(sig[i]!=0 & !is.na(sig[i]) &!is.na(lookfl[i])) {
        conto=conto+1
	if (lookfl[i]/sig[i] < 2 & lookfl[i]/sig[i] > 0.5){
		Sf2=Sf2+1
	 }
	}
        }
...
for (i in seq(length(sig))){
  if(sig[i]!=0 & !is.na(sig[i]) &!is.na(lookfl[i])) {
    conto=conto+1
    if (lookfl[i]/sig[i] < 5 & lookfl[i]/sig[i] > 0.2){
      Sf5=Sf5+1
    }
  }
}
...
meanDi=mean(int,na.rm=TRUE)
meanMi=mean(lookint,na.rm=TRUE)
FBi=(meanDi-meanMi)/(meanDi+meanMi)*2
NMSEi=mean((int-lookint)^2,na.rm=TRUE)/meanDi/meanMi
Ri=cor(int,lookint, use="pairwise.complete.obs")
maxDi=max(int,na.rm=TRUE)
maxMi=max(lookint,na.rm=TRUE)

conto=0
Sf2=0
for (i in seq(length(int))){
         if(int[i]!=0 & !is.na(int[i]) &!is.na(lookint[i])) {
        conto=conto+1
	if (lookint[i]/int[i] < 2 & lookint[i]/int[i] > 0.5){
		Sf2=Sf2+1
	 }
	}
        }
...
for (i in seq(length(int))){
  if(int[i]!=0 & !is.na(int[i]) &!is.na(lookint[i])) {
    conto=conto+1
    if (lookint[i]/int[i] < 5 & lookint[i]/int[i] > 0.2){
      Sf5=Sf5+1
    }
  }
}
...
stat1 = format(stat1,digits = 3,scientific=T)
write.table(stat1,append = F, quote = FALSE)
write.table(stat1,file='fire.txt',append = FALSE, quote = FALSE)


ksp=(1/lookint)^1.086
lambda=gamma(1+1/ksp)/lookcm
R90=(-log(0.1))^(1/ksp)/(lambda*lookcm)
ksp=(1/sig)^1.086
lambda=gamma(1+1/ksp)/mea
R90=(-log(0.1))^(1/ksp)/(lambda*mea)
...
for (i in seq(length(mis[,3]))){
	...
	mis1[i,2]=mis[i,2]	
	mis1[i,3]=mis[i,3]
	cinter1[i,1]=lookcm[i]	
	cinter1[i,2]=lookfl[i]	
	cinter1[i,3]=lookint[i]	
	}else{
    mis2[i,1]=mis[i,1]	
	mis2[i,2]=mis[i,2]	
	mis2[i,3]=mis[i,3]
	cinter2[i,1]=lookcm[i]	
	cinter2[i,2]=lookfl[i]	
	cinter2[i,3]=lookint[i]			
}
}
...
quartz(width=12)
par(mfrow=c(2,3))
 plot(lookcm,mea)#,xlim=c(0,50),ylim=c(0,50),main="Mean",xlab="model",ylab="observations",cex.axis=1.5,cex.lab=1.5)
 abline(0,1)
 ...
 # abline(0,1)
  ...
  qqplot(lookcm,mea)#,xlim=c(0,50),ylim=c(0,50),main="Mean",xlab="model",ylab="observations",cex.axis=1.5,cex.lab=1.5)
 abline(0,1)
...
quartz()
 ...
 plot(lookfl,mis[,2],xlim=c(0,25),ylim=c(0,25),main="Standard deviation",xlab="model",ylab="observations",cex.axis=1.5,cex.lab=1.5)
 abline(0,1)
 plot(lookint,mis[,3],xlim=c(0,3),ylim=c(0,3),main="Intensity",xlab="model",ylab="observations",cex.axis=1.5,cex.lab=1.5)
 abline(0,1)
 qqplot(lookfl,mis[,2],xlim=c(0,25),ylim=c(0,25),main="Standard deviation",xlab="model",ylab="observations",cex.axis=1.5,cex.lab=1.5)
 abline(0,1)
 qqplot(lookint,mis[,3],xlim=c(0,3),ylim=c(0,3),main="Intensity",xlab="model",ylab="observations",cex.axis=1.5,cex.lab=1.5)
 abline(0,1)
...
taylor.diagram(mis[,1],lookcm,col="green",add=F)
taylor.diagram(mis[,2],lookfl,col="magenta",add=F)
taylor.diagram(mis[,3],lookint,col="blue",add=F)
